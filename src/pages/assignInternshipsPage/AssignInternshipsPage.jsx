...
const AssignInternships = () => {
  ...
  const checkPublishedStatus = async () => {
    try {
      const status = await internshipService.checkPublishStatus();
  ...
  const handlePublishPlanning = async () => {
    try {
      ...
      const response = await internshipService.publishPlanning(action);
      setIsPublished(!isPublished);
      showTemporaryMessage(response?.message || `Planning successfully ${isPublished ? "hidden" : "published"}.`, false);
    } catch (error) {
      showTemporaryMessage(error.response?.data?.message || "Error updating planning status.", true);
    }
  };
    ...
    <div className="flex flex-wrap gap-2 mb-6 justify-between items-center">
      ...
      <div className="ml-auto flex items-center">
        ...
        <button
          ...
          className={`${isPublished ? "bg-red-500 hover:bg-red-600" : "bg-purple-500 hover:bg-purple-600"} text-white px-4 py-1.5 rounded-lg text-sm font-medium flex items-center mx-2`}
        >
          {isPublished ? (
            <>
